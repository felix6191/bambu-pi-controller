            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 8)
                        ...
                        .frame(width: Swift.max(0, Swift.min(geo.size.width, geo.size.width * CGFloat(current) / CGFloat(maxTemp))), height: 20)
                        .animation(.easeInOut(duration: 0.3), value: current)
                    if target > 0 {
                        Rectangle().fill(Color.white).frame(width: 2, height: 24)
                            .offset(x: Swift.min(geo.size.width, geo.size.width * CGFloat(target) / CGFloat(maxTemp)) - 1)
                    }
                }
            }.frame(height: 20)
            Slider(value: Binding(get: { Double(target) }, set: { target = Int($0) }), in: 0...Double(maxTemp), step: 1).tint(color)
            HStack(spacing: 8) {
    ...
    var body: some View {
        VStack(spacing: 12) {
            HStack {
                Text(title).font(.headline)
                Spacer()
                Text("\(value)\(unit)").font(.system(.body, design: .rounded)).fontWeight(.bold).foregroundColor(color)
            }
            ...
            HStack {
                ...
                Spacer()
                Text("Aktuell: \(current)\(unit)").font(.caption).foregroundColor(.secondary)
                Spacer()
